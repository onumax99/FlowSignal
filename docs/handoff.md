...
### M2.5 追加仕様（eval / features / models / scripts）
- `eval/baselines.majority_class(labels)` — 多数派ベースライン。**fold ごとの train 最頻クラス**で予測（test を見ない＝リーク無し）。
- `eval/metrics.block_bootstrap_macro_f1(y_true, pred_dict, blocks, reference=...)` — macro-F1 の**日付ブロック** bootstrap（各系列 CI＋reference 差）。
- `eval/calibration.PlattCalibrator(classes)` — 多クラス Platt 較正（**train 末尾の validation で fit**・test では fit しない）。
- `eval/hold` — `select_threshold_for_coverage`（validation 確信度で被覆率目標）/ `covered_metrics` / `coverage_curve`。
- `models/baseline.make_regressor("lightgbm", seed=42)` — クロスセクション相対予測用 LGBM 回帰。
- `features/cross_section` — `add_relative_target`（翌日リターンの日次デミーン）/ `cross_sectional_zscore`（日次 z-score）。
- `eval/cross_section_metrics` — `daily_rank_ic` / `long_short_returns` / `bootstrap_mean_ci`。
- 実行: `train_baseline.py --calibrate`（HOLD 分析）/ `train_cross_section.py`（相対予測）。**いずれも結果は negative**（§5）。
