...
- **M2 の結論（正直に）**: **弱ベースライン（always-up / random / prev-direction）に accuracy で有意**
  ...
  さらに **多数派（always-FLAT≈43%）ベースラインは未掲載で、モデル 41.0% はこれを上回らない見込み**
...
|---|---|---|
| 既定（class_weight=none） | **41.0%** | 34.9% |
| balanced | 39.6% | **37.2%** |
| ベースライン prev-direction | 36.3% | – |
| ベースライン random / always-up | 33.0% / 29.8% | – |

- McNemar（既定）: vs prev-direction **p=7.9e-18**、vs always-up **p=4.6e-74** → いずれも有意。
