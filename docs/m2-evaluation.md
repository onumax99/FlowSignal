...
→ 残課題: J-Quants 経路の adjusted 化（`prices.py` TODO）。**確率較正＋HOLD は §5 で実装・測定済み**。クロスセクション化（要 universe 拡大）は M2.5 残作業。
...
- **精度を上げる次の一手はモデルいじりではない**（[prediction-design.md](prediction-design.md) §4）:
  1. **HOLD（確信度で棄権）＋確率較正** — 実装・測定済み（§5）。**accuracy は上がるが macro-F1 は上がらない**
     （較正の argmax が FLAT に collapse）＝方向 skill は動かない。
  2. **クロスセクション（相対）予測** — 実装・測定済み（§6・universe 30 銘柄）。**rank IC ≈ 0 で edge 無し（negative）**。
  3. **イベント駆動の LLM 新情報（M3 本命）** — 決算・開示・ニュースで価格に未織り込みの edge を取る
- 現状の重要度（マーケット系が支配的）は、上記 2・3 の方向性（相対化・新情報）の妥当性を支持する。

## 5. HOLD（確信度で棄権）＋確率較正（M2.5）

各 fold で **train 末尾 20% を validation** に切り出し、fit 区間で学習→cal 区間で **Platt 較正**→test に適用。
棄権閾値も cal の確信度で被覆率目標に合わせ test に適用（test で選ばない＝リーク無し）。再現: `python scripts/train_baseline.py --calibrate [--class-weight balanced]`。

| target cov | actual cov | accuracy | macro-F1 | covered 予測比 DOWN/FLAT/UP |
|---|---|---|---|---|
| 100% | 100% | 43.6% | 0.224 | 1 / 97 / 1 |
| 80% | 79% | 45.3% | 0.217 | 1 / 99 / 0 |
| 60% | 60% | 47.4% | 0.218 | 0 / 100 / 0 |
| 40% | 40% | 49.1% | 0.219 | 0 / 100 / 0 |

（balanced。既定 class_weight=none も同傾向: 100% で accuracy 43.5% / macro-F1 0.220）

### 結論（正直に・想定どおりの落とし穴）
- **較正の argmax は多数派(FLAT)に collapse する。** Platt 較正は確率を base rate(FLAT 43%)へ寄せるため argmax がほぼ FLAT になり、
  **balanced が raw で得ていた macro-F1 0.372 は較正後 0.224 へ低下**（≈ always-majority 0.202）。
  較正は確率の質（accuracy/log-loss）を整えるが、**macro-F1 を上げる操作ではない**。
- **HOLD は accuracy を上げるが macro-F1 を上げない。** 棄権すると covered set が FLAT に偏り（予測比 97→100% FLAT）、
  accuracy は 43.6→49.1% と自動上昇するが **macro-F1 は ~0.22 で横ばい**。
  prediction-design §4①／指摘⑤「棄権は FLAT 偏りで accuracy だけ上がる」を実データで確認。
- **方向 skill（macro-F1）に効いた唯一のレバーは raw balanced(0.372)**。較正・HOLD の後処理では動かない。
  → 価格だけでは頭打ちで、**新情報（M3: LLM×開示）が本命**という設計仮説（[prediction-design.md](prediction-design.md) §3・§4③）を補強。
- 補足: 較正確率を保ちつつ macro-F1 を上げるには argmax でなく**クラス別閾値/コスト感応な決定規則**が要る（今回のスコープ外・M3 後に検討）。

## 6. クロスセクション（相対）予測（M2.5）

universe を **30 銘柄**に拡大し、絶対方向でなく「同日内の相対強弱」を予測（目的＝翌日リターンの日次デミーン、
特徴量＝日次 z-score、LightGBM 回帰、日付境界 walk-forward）。評価は rank IC とロングショート。
再現: `python scripts/train_cross_section.py`。

| 系列 | mean IC | ICIR | t | 正の日 | 95%CI(日ブートストラップ) |
|---|---|---|---|---|---|
| モデル | -0.0024 | -0.011 | -0.36 | 50% | [-0.016, +0.011] |
| ランダム | -0.0026 | -0.014 | -0.44 | 50% | – |
| モメンタム(ret_5) | -0.0131 | -0.049 | -1.54 | 48% | – |

ロングショート（上位/下位 20%）: 日次スプレッド **-4.7bp** / Sharpe(√252) **-0.65**（n_days=1000）。

### 結論（正直に・negative）
- **クロスセクションの edge は検出されず。** モデルの mean IC は -0.0024 ≈ ランダム(-0.0026) で、
  **95%CI が 0 を跨ぎ有意でない**。ロングショートも負（-4.7bp / Sharpe -0.65）で skill なし。
- テクニカルのみの cs 特徴量では、日次・大型株の相対方向に予測力が無い（モメンタムも弱い reversal だが非有意）。
- HOLD（§5）に続く negative。**価格・テクニカルの後処理／相対化では edge は作れない**ことを 3 手法
  （HOLD・確率較正・クロスセクション）で一貫して確認。→ 残るレバーは **新情報（M3: LLM×ニュース/開示）**。
  M2.5 の一連の negative はこの設計仮説（[prediction-design.md](prediction-design.md) §3・§4③）を強く支持する。
- 注意: 30 銘柄は **現在も上場する大型株**から選んだため survivorship bias あり（PoC では許容）。

> 注: McNemar は accuracy（正誤の 2×2）に対する検定で macro-F1 は対象外。fold 間 mean±std は
> fold が相関するため厳密な検定ではない（PoC のヒューリスティック）。rank IC の有意性は日ブートストラップで見る。
