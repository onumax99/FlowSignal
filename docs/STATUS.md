...
- [x] M2.5: クロスセクション（相対）予測＝`features/cross_section.py`（日次デミーン目的＋日次 z-score）/`models/baseline.make_regressor`/`eval/cross_section_metrics.py`（rank IC・ロングショート・日ブートストラップ）/`scripts/train_cross_section.py`＋単体テスト8件（計76件緑）。**結果は negative: mean IC=-0.0024≈ランダム・95%CI が 0 跨ぎ非有意、ロングショート -4.7bp/Sharpe -0.65**（[m2-evaluation.md](m2-evaluation.md) §6）（2026-06-19）
