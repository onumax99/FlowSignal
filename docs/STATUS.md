...
| 仮想環境 | `.venv`（リポジトリ直下、git 管理外）。有効化: `.venv\Scripts\Activate.ps1`。**現状この Windows 機には未作成** |
...
> ※ 具体的な版は**環境構築後に `pip freeze` で確定**させる。旧 macOS/py3.9 venv の版固定（numpy 2.0.2 等）は py3.13 では解決結果が異なる（より新しい numpy 等になる）ため引き写さないこと。

### 環境メモ（Windows）

1. **lightgbm はそのまま動く想定（macOS の libomp 問題は無い）**
   - Windows 版 wheel は OpenMP ランタイムを同梱するため、`pip install -e ".[ml]"` 後に `import lightgbm` が**追加導入なしで通る**見込み。初回構築時に import を一度確認すること。
...
- [ ] テクニカル指標の単体テスト（既知値照合＋先読み無し[shift整合]のアサート）※自前実装のリーク制御を担保
