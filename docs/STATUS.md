...
| M2.5 | 評価の honest 化＋低コスト改善（API 不要） | 多数派＋全ベースラインの macro-F1 併記・macro-F1 の bootstrap 有意性検定／HOLD＋確率較正／クロスセクション化 | 🚧 進行中（honest 化 ✅ 完了・残: HOLD＋較正/クロスセクション） |
...
> **M2 完了**（2026-06-17、2026-06-18 に adjusted close 再実行）。pytest 全 58 件グリーン。**M2.5 の評価 honest 化は完了**、残りは HOLD＋確率較正・クロスセクション（API 不要）。
