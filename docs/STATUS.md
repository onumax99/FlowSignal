...
src/flowsignal/
  ...
  models/
    baseline.py               lightgbm 既定の薄いファクトリ（seed固定・hist代替可・M2 で追加）
  eval/
    split.py                  日付境界 walk-forward 分割（M2 で追加）
    baselines.py              always-up / random / prev-direction（M2 で追加）
    metrics.py                accuracy/macro-F1/per-class/混同行列＋McNemar（M2 で追加）
scripts/fetch_daily.py        日次取得エントリポイント
scripts/train_baseline.py     M2 学習〜評価の通し実行（M2 で追加）
...
| M2.5 | 評価の honest 化＋低コスト改善（API 不要） | 多数派＋全ベースラインの macro-F1 併記・macro-F1 の bootstrap 有意性検定／HOLD＋確率較正／クロスセクション化 | 🚧 次の本命 |
...
- [x] M2: `features/labels.py` 3クラスラベル（**既定=ボラ連動 k×HV20, k=0.5**・fixed 切替可・σ は t 時点まで）。実データ分布 DOWN27.2/FLAT43.0/UP29.8%
...
> **M2 完了**（2026-06-17）。pytest 全 50 件グリーン。次は M3（LLM ニュース特徴量）。
...
- **評価の honest 化（M2.5 で実施）**: 多数派(always-FLAT≈43%)ベースライン未掲載・全ベースラインの macro-F1 未算出・macro-F1 の有意性検定（**日次/時間ブロックの block bootstrap**、5 fold の再標本化ではない）未実施。現 M2 の「accuracy で有意」は弱ベースライン限定の主張。`eval/baselines.py` への多数派追加＋ baselines の macro-F1 併記（指標で強弱が逆転: always-FLAT の macro-F1 ~0.20、prev-direction ~0.33-0.36 が本命対戦相手）＋ bootstrap CI で是正（[m2-evaluation.md](m2-evaluation.md)「限界・追加検証」）。
...
- 🆕 **株価の分割・配当調整が未対応（M2 数値に影響し得る）**: `data/prices.py` は yfinance `auto_adjust=False`＋生 `Close` 取得。株式分割・配当で close-to-close リターンが跳ね、偽の極端リターン＝偽ラベル/特徴量（hv20 経由でラベル閾値も歪む）を生む。**adjusted close へ切替＋ M2 再実行**で是正する（[m2-evaluation.md](m2-evaluation.md) 限界・[requirements.md](requirements.md) §6）。
...
- **HOLD＋確率較正・クロスセクション（相対）予測**: [prediction-design.md](prediction-design.md) §4①②の最優先レバー。**M2.5（API 不要）で正式に実施**することに決定（M2 はテクニカル単独 pooled に限定）。lead-lag は②③の枠内の一特徴として後続で検討。
