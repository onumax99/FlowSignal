...
| M2.5 | 評価の honest 化＋低コスト改善（API 不要） | 多数派＋全ベースラインの macro-F1 併記・macro-F1 の bootstrap 有意性検定／HOLD＋確率較正／クロスセクション化 | 🚧 進行中（honest 化 ✅・HOLD＋較正 ✅(macro-F1 は不変=negative)・残: クロスセクション） |
...
> **M2 完了**（2026-06-17、2026-06-18 に adjusted close 再実行）。pytest 全 68 件グリーン。**M2.5 の honest 化＋HOLD＋較正は完了**（HOLD は macro-F1 を動かさず=negative）、残りはクロスセクション（要 universe 拡大・API 不要）。
...
- **HOLD＋確率較正は実施・完了（negative）**: 較正の argmax が FLAT collapse で macro-F1 は不変（[m2-evaluation.md](m2-evaluation.md) §5）。**クロスセクション（相対）予測**が M2.5 の残レバー（要 universe 拡大・API 不要）。lead-lag は②③の枠内の一特徴として後続で検討。
