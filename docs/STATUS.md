...
- 🚨 **【M3 ブロッカー】ニュース/開示の履歴データ確保**: `data/news.py` は RSS のみで過去アーカイブを返さず、M1/M2 は `--skip-news` で実行済み＝5 年分のニュース履歴が無い。M3 を M2 と同じ walk-forward で評価するには過去ニュースが必要。**forward 限定評価**か **TDnet 等の履歴ソース確保**かを M3 設計前に決める（[handoff.md](handoff.md) §5・§6 / [requirements.md](requirements.md) §13）。
- **評価の honest 化（M2.5 で実施）**: 多数派(always-FLAT≈43%)ベースライン未掲載・全ベースラインの macro-F1 未算出・macro-F1 の有意性検定（fold ブロック bootstrap）未実施。現 M2 の「accuracy で有意」は弱ベースライン限定の主張。`eval/baselines.py` への多数派追加＋ baselines の macro-F1 併記＋ bootstrap CI で是正（[m2-evaluation.md](m2-evaluation.md)「限界・追加検証」）。
...
- 対象銘柄リストの確定（現状 universe.yaml に大型15銘柄）。クロスセクション/lead-lag を狙うなら **銘柄数拡大（30+）が効く**（API 不要・M2.5 で前倒し可）。
