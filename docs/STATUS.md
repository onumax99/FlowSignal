...
- [x] M2.5: クロスセクション（相対）予測＝`features/cross_section.py`（日次デミーン目的＋日次 z-score）/`models/baseline.make_regressor`/`eval/cross_section_metrics.py`（rank IC・ロングショート・日ブートストラップ）/`scripts/train_cross_section.py`＋単体テスト8件（計76件緑）。**結果は negative: mean IC=-0.0024≈ランダム・95%CI が 0 跨ぎ非有意、ロングショート -4.7bp/Sharpe -0.65**（[m2-evaluation.md](m2-evaluation.md) §6）（2026-06-19）- [x] M3 step ①（課金前スキャフォールド＋count_tokens 実測 2026-06-19）: `config.anthropic_api_key()`＋`anthropic` 依存（`llm` extra・実測 0.111.0）＋`scripts/measure_tokens.py`（count_tokens＝**無料**で §6 実測・cache 最小検証つき）＋単体テスト3件（計 79 件緑）。
  **実測結果**: 固定プレフィックス 745 tok(Opus)/713(Sonnet・Haiku)＝**全モデルの cache 最小未満→prompt caching 不使用・Batch(50%)を主レバーに**。可変入力 ~67 tok(短サンプル・実記事で再測)。1 記事 Opus ~$0.0091。**$5 予算では履歴一括(6,000 件)は全モデル赤字→履歴は数十件の配管検証のみ・forward-only＋Batch を確定**（[m3-design.md](m3-design.md) §6 反映済み）。残: モデル選定（runway 直結＝ユーザー判断）／材料イベント発生率の実測（step ② のデータ待ち）。
...
- ✅ **適時開示(TDnet)の実在性（2026-06-19 決着）**: 履歴(5年)は**有料のみ**（公式 API／J-Quants アドオン ¥11,000 月・個人限定）、**forward は release.tdnet.info の無料 31 日窓で取得可**。→ M3 は **forward-only を無料経路で**実装（履歴一括は $5 赤字でやらない）。次の **step ② で `data/disclosure.py`（ソース=release.tdnet.info）** を実装する（[handoff.md](handoff.md) §5・[m3-design.md](m3-design.md) §8）。
