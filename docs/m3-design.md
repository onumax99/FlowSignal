...
- ④ ⚠️ **コストの単位（採点記事数）と検定力の単位（材料のある (date,code) 行数）は別物**。`relevant_codes` は 0〜数件で **多くの記事は universe 30 銘柄に紐づかない**ため、非ゼロ news 特徴量を持つ行数はコスト前提の記事数よりずっと小さい。**§5.1 の開封点 N_min を決めるのは後者**であり、§6.3 で発生率を実測する。
...
- ④ **材料イベント発生率の実測（検定力＝開封点 N_min の根拠）**: count_tokens と同時に、サンプル期間で「**非ゼロ news 特徴量を持つ (date,code) 行 / 全行**」を数え、forward で N_min（材料のある行が数百以上）に到達するおおよその所要日数を見積もる。§5.1 の開封点・評価日はこの実測で確定して凍結する。
